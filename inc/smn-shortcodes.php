...
function sumun_shortcode_carrusel( $atts ) {
	...
	if ( $q->have_posts() ) {
		...
		if ( $fullwidth ) {
			$r .= '<div class="alignfull slick-white-arrows">';
		}
		...
		while ( $q->have_posts() ) { $q->the_post();
			...
			ob_start();
			if ( $fullwidth ) {
				get_template_part( 'loop-templates/content', 'slide-background' );
			} else {
...
add_shortcode( 'carrusel', 'sumun_shortcode_carrusel' );

function sumun_shortcode_slider_ultimas_entradas( $atts ) {
	...
	if ( $q->have_posts() ) {
		...
		$r .= '<div class="alignfull">';

			$r .= '<div class="slick-slider header-slider">';

			while ( $q->have_posts() ) { $q->the_post();

				ob_start();
				get_template_part( 'loop-templates/content', 'slide-header' );
				$r .= ob_get_clean();
...
add_shortcode( 'slider_ultimas_entradas', 'sumun_shortcode_slider_ultimas_entradas' );